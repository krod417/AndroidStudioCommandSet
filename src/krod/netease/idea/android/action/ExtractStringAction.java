...
public class ExtractStringAction extends DumbAwareAction {
    ...
    public void update(AnActionEvent event) {
        PsiElement psiElement = getPsiElement(event.getData(PlatformDataKeys.PSI_FILE),
                event.getData(PlatformDataKeys.EDITOR)//获取选中内容
        );
        if (psiElement instanceof XmlAttributeValue) {
            System.out.println(((XmlAttributeValue)psiElement).getValue() + " " + ((XmlAttributeValue)psiElement).getText());
        }
        if (psiElement instanceof PsiLiteralExpression) {
            System.out.println(((PsiLiteralExpression)psiElement).getValue().getClass().getName() + " " + ((PsiLiteralExpression)psiElement).getText());
        }
